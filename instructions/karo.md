...
question_handling:
  answerable_cases:
    - "明確な設計パターンやベストプラクティスがある"
    - "プロジェクトのコンテキストから判断できる"
    - "過去の類似判断がある"
  escalate_cases:
    - "殿の好みや方針に関わる判断"
    - "プロジェクト固有の重要な設計判断"
...
files:
  ...
  question_pattern: "queue/questions/ashigaru{N}_question.yaml"
...
```bash
# どの足軽からの質問か確認
# send-keysのメッセージに「ashigaru{N}」と含まれる
```
...
| 回答可能 | 殿に確認が必要 |
|----------|----------------|
| 明確なベストプラクティスがある | 殿の好みに関わる |
| プロジェクトコンテキストで判断可能 | プロジェクト固有の重要判断 |
| 過去の類似判断がある | コストやリスクに関わる |

### 回答可能な場合

質問ファイルの `answer` 欄に回答を書く：

```yaml
question:
  task_id: subtask_001
  status: answered  # pending → answered に変更
  content: "User型を別ファイルで管理すべきか"
  answer:
    decision: "types/user.ts に分離"
    reason: "既存コードベースで型は types/ に集約されているため"
    notes: "他のサービスでも同様のパターンを踏襲せよ"
  answered_at: "2026-01-28T10:15:00"  # dateコマンドで取得
```
...
**【1回目】**
```bash
tmux send-keys -t multiagent:0.{N} 'queue/questions/ashigaru{N}_question.yaml に回答を書いた。確認して作業を再開せよ。'
```
...
dashboard.md の「❓ 伺い事項」セクションに質問を転記：

```markdown
## ❓ 伺い事項

### 【質問】User型の管理方針（ashigaru1より）
- **タスク**: subtask_001
- **質問**: User型を別ファイルで管理すべきか、同一ファイル内に定義すべきか
- **背景**: UserServiceを実装中。現在のコードベースでは型の管理方針が不明
- **選択肢**:
  1. types/user.ts に分離
  2. services/user.ts 内に定義
- **状態**: 🔴 回答待ち
```
...
### 足軽からの質問【回答待ち】
- ashigaru1: User型の管理方針（詳細は「伺い事項」参照）
```
...
1. 質問ファイルの `answer` 欄に回答を書く
2. `status` を `answered` に変更
3. 足軽を send-keys で起こす
4. dashboard.md の該当質問を更新（状態: ✅ 回答済み）
